	...
	)
{
	...
	// @method formatQuantity Formats a non-negative number with commas as thousand separator (e.g. for displaying quantities)
	...
	// @method substitute @param {String} text @param {Object} object containing key/values to replace. @return {String} the resulting string of making the substitution
	...
	// @param {String} options_string @return {Object}
	...
	// @method isTargetActionable @param {Event} event @return {Boolean}
	...
	// @method findItemInCart Algorithm to search an item in the cart. We can't use internalid only because for matrix items, the internalid change when is added to the cart
	// @param {ItemDetails.Model} findItem @param {LiveOrder.Model} cart
	...
	// @method require The motive for this method is being able to call require() of modules inline withtout breaking amd-optimizer because we cannot use require() directly passing variables as dependencies because amd-optimizer will try to parse that and fail.
	// @parma {Array<String>} dependencies @param {Function} fn
	// function requireModules (dependencies, fn)
	// {
	// 	require(dependencies, fn);
	// }
	var requireModules = typeof require === 'undefined' ? undefined : require;
	if (window.SCM)
	{
