	...
	,	registerNavigationEvents: function(application)
		{
			...
			//TODO: application.getLayout().touch* ? test mobile

			// the following comments are 2 other means of registering that improves in concrete aspects but breaks things / add complexity : 

			// registering mousedown DOM event in the currentView. 
			// necessary on any navigation with buttons, like checkout-steps , update quantity
			application.getLayout().on('afterAppendView', function (view)
			{
				...
				// Heads up! view.$el.unbind() is called automatically 
			});

			// register to a generic url change. needed when only the hash parameters changed - concretely the list header. 
			// For some reason Backbone.history.on doesn't work there. Example: #reorderItems?filter=2016-04-22T2016-06-21
			// REMOVED: this is breaking checkout step navigation - fix ListHeader
			// window.onpopstate = _.bind(this.handleNavigationEventStart, this); 
		}
