	...
	)
{
	...
	_.extend(Backbone.View.prototype, {
		...
		// @method showError @param {String} message @param {String} type
	,	showError: function (message, type, closable)
		{
			this.hideError();
	...
	,	mountToApp: function (application)
		{
			...
			_.extend(Layout, {
				...
				// They will be use to try to get the error message of a faild ajax call. Extend this as needed
			...
			,	unauthorizedError: function (user_session_timedOut)
				{
					...
					var base_url = _.setUrlParameter(Session.get('touchpoints.login'), 'origin', Configuration.currentTouchpoint);
					base_url = _.setUrlParameter(base_url, 'origin_hash', Backbone.history.fragment);
						Configuration.currentTouchpoint = 'login';

					window.location = user_session_timedOut ?
										_.setUrlParameter(base_url, 'timeout', 'T') :
										base_url;
						}
			});
			...
			jQuery(document).ajaxError(function (e, jqXhr, options, error_text)
			{
				...
				{
					Layout.unauthorizedError(jqXhr.responseJSON.errorCode === 'ERR_USER_SESSION_TIMED_OUT');
					return;
				}
				...
				if (!jqXhr.preventDefault)
				{
					...
					else if (Layout.currentView)
					{
						...
						if (intStatus !== 403)
						{
							...
							{
								var activeView = _.find(Layout.currentView.childViewInstances, function(childView)
								{
									return childView && (childView.$('.focused-form-view').size() || childView.$el.is('.focused-form-view'));
								});
