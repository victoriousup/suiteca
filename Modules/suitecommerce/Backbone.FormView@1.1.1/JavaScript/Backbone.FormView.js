	...
	)
{
	...
	return {
		...
		add: function (view)
		{
			...
			view.clonedModel = view.model.clone();
			view.clonedModel.set(view.model.attributes, {silent:true});
			view.model.on('change', this.synchronizeModels, view);

			// override render() to add support to double binding using Backbone.stickit:
			view.render = _.wrap(view.render, function (fn)
			{
				...
				this.stickit(this.clonedModel, this.bindings);
	...
	,	synchronizeModels: function (view_model)
		{
			...
			{
				view.clonedModel.set(attribute, value);
			});
		}
