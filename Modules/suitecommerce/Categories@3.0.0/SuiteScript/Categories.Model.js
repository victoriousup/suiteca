	...
	)
{
	...
	,	categoryColumns: {
		...
		,	category: {
				...
				,	'idpath' // need it for the breadcrumb
				,	'fullurl' // need it for the canonical path
				,	'isprimaryurl' // need it for the canonical path
				]
			}
	...
	,	getColumns: function (element)
		{
			// TODO: change to Configuration.get() from Configuration branch;
	...
	,	get: function (fullurl)
		{
			...
			{
				var categories = this.getCategories(category.parenturl, { 'internalid': category.internalid, 'fullurl': fullurl }, this.getColumns('category'));
	...
	,	getCategoriesOverride: function (parenturl, categoryid, columns)
		{
			...
			,	searchFilters = [
				...
				//,	new nlobjSearchFilter('subcatdisplayinsiteoverride', null, 'is', 'T')
				]
			,	searchColumns = []
	...
	,	getBreadcrumb: function (idpath)
		{
			...
			if (filters.length > 1)
			{
				...
				{
					// if (!index && crumb.getValue('displayinsite') === 'F')
					// {
					// 	throw notFoundError;
					// }

					if (crumb.getValue('isinactive') === 'T') // || (index !== result.length - 1 && crumb.getValue('subcatdisplayinsiteoverride') === 'F'))
					{
						throw notFoundError;
					}
			...
			Application.getAllSearchResults('commercecategory', filters, searchColumns).forEach(function (line)
			{
				...
				if (!bag[idPath])
				{
					...
					_.each(columns, function(column)
					{
						...
						// else
						// {
							// current.displayinsite = override ? override.displayinsite : line.getValue('displayinsite');
						// }
					});
	...
	,	sortBy: function (categories, property)
		{
			if (categories)
			{
				...
				{
					self.sortBy(category.categories);
				});
