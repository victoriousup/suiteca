...
Available options:

	{
		...
		//  TODO: ,	appendPusher: true/false -> if no pusher button is added, this will prepend a pusher button next to the content
	} 	
	...
	)
{
	...
	jQuery.fn.scPush = function (options)
	{
		...
		if(opened_pusher.length > 0)
		{
			...
			// TODO: scroll back to original position
		}
		...
		return this.each(function() 
		{
			...
			var push_header = jQuery('<div class="sc-pusher-header"><a href="#" class="sc-pusher-header-back" data-action="sc-pusher-dismiss">' + _('Back').translate() + '</a></div>');
			...
			// add push button if needed
			if(settings.appendPusher)
			{
				// TODO: add push button (with id as target)
				console.log('TODO: append pusher');
			}
			...
			// open push action
			pusher.on('click', jQuery.proxy(function(e)
			{
				e.preventDefault();
				...
				jQuery.proxy(openPushContent, this)(id);

			}, this));

			// close push action
			push_header.find('[data-action="sc-pusher-dismiss"]').on('click', jQuery.proxy(function(e)
			{
				e.preventDefault();
	...
	{	
		if (Utils.getViewportWidth() >= 768)
		{
			...
			overlay.on('click', function()
			{
				...
				// TODO: check if following commented block was really necessary
				// // once the push closes, scroll back to the top
				// pushed_content.one('webkitTransitionEnd otransitionend oTransitionEnd msTransitionEnd transitionend', function (e) {
