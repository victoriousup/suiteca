	...
	)
{
	...
	,	rate: function (e, rater)
		{
			...
			if (~_.indexOf(attributes_to_rate_on, rater.name))
			{
				...
				{
					this.model.validate(rater.name);
				}
			...
			else if (rater.name === '__overall__')
			{
				...
				this.clonedModel.set('rating', this.tmpRating);
				this.clonedModel.validate('rating');
			}
	...
	,	preview: function (e)
		{
			...
			{
				new ProductReviewsFormPreviewView(_.extend({ formView: this }, this.options)).showContent();
			}
		}
	...
	,	prepareItem: function ()
		...
		//@method save dispatch the user 'save' action. IIt will validate the form and then show the review preview
	,	customSaveForm: function (e)
		{
			...
			promise && promise.done(function ()
			{
				...
				var preview_review = new ProductReviewsFormConfirmationView(self.options);
	...
	,   updateMetaTags: function ()
		...
		//@method getContext @returns {ProductReviews.Form.View.Context}
	,	getContext: function()
		{
