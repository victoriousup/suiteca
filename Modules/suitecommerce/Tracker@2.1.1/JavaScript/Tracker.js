	...
	)
{
	...
	Tracker.prototype.trackAddToCart = function (item, event)
	{
	...
	Tracker.prototype.trackProductView = function (item)
	{
		if (!this.trackOnce['view_' + item.get('_sku', true)])
		{
			this.trackOnce['view_' + item.get('_sku', true)] = true;

			return this.track('trackProductView', item);
		}
